...
$album_seguiti_query = "SELECT album.id AS id, album.nome AS nome, album.descrizione AS descrizione, album.data AS data, album.imgLink AS imgLink, u.nome AS nome_seguito, u.imgProfilo AS profilo_seguito, idea.testo AS testo, idea.data AS data_idea FROM Utente INNER JOIN follow f ON (f.seguente=Utente.email) INNER JOIN Utente u ON (u.email=f.seguito) INNER JOIN Album ON (album.email = u.email) INNER JOIN idea ON (idea.autore=Utente.nome) WHERE Utente.nome='$user' AND idea.autore='$user' AND privato=0 ORDER BY data DESC LIMIT 6";
